...
import com.wartechwick.sudoku.db.SudokuColumns;
import com.wartechwick.sudoku.db.SudokuDatabase;
import com.wartechwick.sudoku.game.CellCollection;
import com.wartechwick.sudoku.game.FolderInfo;
import com.wartechwick.sudoku.game.SudokuGame;
import com.wartechwick.sudoku.utils.AndroidUtils;
...
public class SudokuListActivity extends ListActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(com.wartechwick.sudoku.R.layout.sudoku_list);
		mFilterStatus = (TextView) findViewById(com.wartechwick.sudoku.R.id.filter_status);
		...
		mAdapter = new SimpleCursorAdapter(this, com.wartechwick.sudoku.R.layout.sudoku_list_item,
				...
				new int[]{com.wartechwick.sudoku.R.id.sudoku_board, com.wartechwick.sudoku.R.id.state, com.wartechwick.sudoku.R.id.time,
						com.wartechwick.sudoku.R.id.last_played, com.wartechwick.sudoku.R.id.created, com.wartechwick.sudoku.R.id.note});
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		menu.add(0, MENU_ITEM_FOLDERS, 0, com.wartechwick.sudoku.R.string.folders).setShortcut('1', 'f')
				.setIcon(com.wartechwick.sudoku.R.drawable.ic_sort);
		menu.add(0, MENU_ITEM_FILTER, 1, com.wartechwick.sudoku.R.string.filter).setShortcut('1', 'f')
				.setIcon(com.wartechwick.sudoku.R.drawable.ic_view);
		menu.add(0, MENU_ITEM_INSERT, 2, com.wartechwick.sudoku.R.string.add_sudoku).setShortcut('3', 'a')
				.setIcon(com.wartechwick.sudoku.R.drawable.ic_add);
...
//		.setIcon(R.drawable.ic_add);
		...
		switch (id) {
			case DIALOG_DELETE_PUZZLE:
				return new AlertDialog.Builder(this).setIcon(
						com.wartechwick.sudoku.R.drawable.ic_delete).setTitle("Puzzle").setMessage(
						com.wartechwick.sudoku.R.string.delete_puzzle_confirm)
			...
			case DIALOG_EDIT_NOTE:
				...
				final View noteView = factory.inflate(com.wartechwick.sudoku.R.layout.sudoku_list_item_note,
						null);
				mEditNoteInput = (TextView) noteView.findViewById(com.wartechwick.sudoku.R.id.note);
				return new AlertDialog.Builder(this).setIcon(
						com.wartechwick.sudoku.R.drawable.ic_add).setTitle(com.wartechwick.sudoku.R.string.edit_note)
						.setView(noteView).setPositiveButton(com.wartechwick.sudoku.R.string.save,
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog,
														int whichButton) {
			...
			case DIALOG_RESET_PUZZLE:
				return new AlertDialog.Builder(this).setIcon(
						com.wartechwick.sudoku.R.drawable.ic_restore).setTitle("Puzzle")
						.setMessage(com.wartechwick.sudoku.R.string.reset_puzzle_confirm)
						.setPositiveButton(android.R.string.yes,
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog,
														int whichButton) {
			...
			case DIALOG_FILTER:
				return new AlertDialog.Builder(this)
						.setIcon(com.wartechwick.sudoku.R.drawable.ic_view)
						.setTitle(com.wartechwick.sudoku.R.string.filter_by_gamestate)
						.setMultiChoiceItems(
								com.wartechwick.sudoku.R.array.game_states,
								new boolean[]{
	...
	public void onCreateContextMenu(ContextMenu menu, View view,
		...
		menu.add(0, MENU_ITEM_PLAY, 0, com.wartechwick.sudoku.R.string.play_puzzle);
		menu.add(0, MENU_ITEM_EDIT_NOTE, 1, com.wartechwick.sudoku.R.string.edit_note);
		menu.add(0, MENU_ITEM_RESET, 2, com.wartechwick.sudoku.R.string.reset_puzzle);
		menu.add(0, MENU_ITEM_EDIT, 3, com.wartechwick.sudoku.R.string.edit_puzzle);
		menu.add(0, MENU_ITEM_DELETE, 4, com.wartechwick.sudoku.R.string.delete_puzzle);
	}
	...
	private void updateFilterStatus() {
		...
		if (mListFilter.showStateCompleted && mListFilter.showStateNotStarted && mListFilter.showStatePlaying) {
			...
			mFilterStatus.setText(getString(com.wartechwick.sudoku.R.string.filter_active, mListFilter));
	...
	private static class SudokuListViewBinder implements ViewBinder {
		...
		public boolean setViewValue(View view, Cursor c, int columnIndex) {
			...
			switch (view.getId()) {
				case com.wartechwick.sudoku.R.id.sudoku_board:
					...
					break;
				case com.wartechwick.sudoku.R.id.state:
					label = ((TextView) view);
					String stateString = null;
					switch (state) {
						case SudokuGame.GAME_STATE_COMPLETED:
							stateString = mContext.getString(com.wartechwick.sudoku.R.string.solved);
							break;
						case SudokuGame.GAME_STATE_PLAYING:
							stateString = mContext.getString(com.wartechwick.sudoku.R.string.playing);
							break;
					}
					...
					break;
				case com.wartechwick.sudoku.R.id.time:
					...
					break;
				case com.wartechwick.sudoku.R.id.last_played:
					long lastPlayed = c.getLong(columnIndex);
					label = ((TextView) view);
					String lastPlayedString = null;
					if (lastPlayed != 0) {
						lastPlayedString = mContext.getString(com.wartechwick.sudoku.R.string.last_played_at,
					...
					break;
				case com.wartechwick.sudoku.R.id.created:
					long created = c.getLong(columnIndex);
					label = ((TextView) view);
					String createdString = null;
					if (created != 0) {
						createdString = mContext.getString(com.wartechwick.sudoku.R.string.created_at,
					...
					break;
				case com.wartechwick.sudoku.R.id.note:
		...
		private String getDateAndTimeForHumans(long datetime) {
			...
			if (date.after(today)) {
				return mContext.getString(com.wartechwick.sudoku.R.string.at_time, mTimeFormatter.format(date));
			} else if (date.after(yesterday)) {
				return mContext.getString(com.wartechwick.sudoku.R.string.yesterday_at_time, mTimeFormatter.format(date));
			} else {
				return mContext.getString(com.wartechwick.sudoku.R.string.on_date, mDateTimeFormatter.format(date));
			}
