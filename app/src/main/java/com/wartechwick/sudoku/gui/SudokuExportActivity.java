...
import com.wartechwick.sudoku.gui.exporting.FileExportTask;
import com.wartechwick.sudoku.gui.exporting.FileExportTaskParams;
import com.wartechwick.sudoku.gui.exporting.FileExportTaskResult;
...
//	public static final String EXTRA_SUDOKU_ID = "SUDOKU_ID";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(com.wartechwick.sudoku.R.layout.sudoku_export);

		mFileNameEdit = (EditText) findViewById(com.wartechwick.sudoku.R.id.filename);
		mDirectoryEdit = (EditText) findViewById(com.wartechwick.sudoku.R.id.directory);
		mSaveButton = (Button) findViewById(com.wartechwick.sudoku.R.id.save_button);
...
//			mExportParams.sudokuID = intent.getLongExtra(EXTRA_SUDOKU_ID, 0);
		...
		switch (id) {
			case DIALOG_FILE_EXISTS:
				...
				return new AlertDialog.Builder(this)
						.setTitle(com.wartechwick.sudoku.R.string.export)
						.setMessage(com.wartechwick.sudoku.R.string.file_exists)
			...
			case DIALOG_PROGRESS:
				...
				progressDialog.setTitle(com.wartechwick.sudoku.R.string.app_name);
				progressDialog.setMessage(getString(com.wartechwick.sudoku.R.string.exporting));
	...
	private void exportToFile() {
		...
		if (!sdcard.exists()) {
			Toast.makeText(SudokuExportActivity.this, com.wartechwick.sudoku.R.string.sdcard_not_found, Toast.LENGTH_LONG);
			finish();
		}
	...
	private void startExportToFileTask() {
		mFileExportTask.setOnExportFinishedListener(new FileExportTask.OnExportFinishedListener() {
			...
			public void onExportFinished(FileExportTaskResult result) {
				...
				if (result.successful) {
					Toast.makeText(SudokuExportActivity.this, getString(com.wartechwick.sudoku.R.string.puzzles_have_been_exported, result.file), Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(SudokuExportActivity.this, getString(com.wartechwick.sudoku.R.string.unknown_export_error), Toast.LENGTH_LONG).show();
				}
				finish();
			}
		});
