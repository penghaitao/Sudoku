...
package com.wartechwick.sudoku.db;
...
import com.wartechwick.sudoku.gui.SudokuListFilter;
...
public class SudokuDatabase {
	...
	public Cursor getSudokuList(long folderID, SudokuListFilter filter) {
		...
		if (folderID != -1) {
			qb.appendWhere(SudokuColumns.FOLDER_ID + "=" + folderID + " and ");
		}

		if (filter != null) {
			if (!filter.showStateCompleted) {
				qb.appendWhere(SudokuColumns.STATE + "!=" + SudokuGame.GAME_STATE_COMPLETED);
			}
    ...
    public Cursor getSudokuListArrangeByLastPlayed(long folderID, SudokuListFilter filter) {
        SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
        ...
        SQLiteDatabase db = mOpenHelper.getReadableDatabase();
        return qb.query(db, null, null, null, null, null, "last_played DESC");
    }

    public long getFolderId(long folderID, SudokuListFilter mListFilter) {
        long folderId = -1;
        Cursor cursor = getSudokuListArrangeByLastPlayed(folderID, mListFilter);
        cursor.moveToNext();
        int index = cursor.getColumnIndex(SudokuColumns.FOLDER_ID);
        if (index != -1 && cursor.getCount() > 0) {
            folderId = cursor.getInt(index);
        }
        cursor.close();
	...
	public long getSudokuId(long folderID, SudokuListFilter mListFilter) {
		long sudoKuId = -1;
		Cursor cursor = getSudokuList(folderID, mListFilter);
		cursor.moveToNext();
		int index = cursor.getColumnIndex(SudokuColumns._ID);
		if (index != -1 && cursor.getCount() > 0) {
			sudoKuId = cursor.getInt(index);
		}
		cursor.close();
	...
	public void updateSudoku(SudokuGame sudoku) {
		ContentValues values = new ContentValues();
		values.put(SudokuColumns.DATA, sudoku.getCells().serialize());
		values.put(SudokuColumns.LAST_PLAYED, System.currentTimeMillis());
		values.put(SudokuColumns.STATE, sudoku.getState());
		values.put(SudokuColumns.TIME, sudoku.getTime());
		values.put(SudokuColumns.PUZZLE_NOTE, sudoku.getNote());
